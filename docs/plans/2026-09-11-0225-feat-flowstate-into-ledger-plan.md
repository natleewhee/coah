...
*Not yet built.* The Product Contract above is now settled (all Outstanding Questions resolved), but a full Planning Contract — Key Technical Decisions on the module's component structure, exactly how `calcInvestmentCapacity`'s inputs flow from the new module into the existing scenario projection, and per-unit Implementation Units with file lists and test scenarios — is a substantial investigation in its own right (the Scenario Planner plan's own Planning Contract runs to ~9 Implementation Units). That work has not been done in this pass; this document remains `artifact_readiness: requirements-only` until it is.

## Post-Plan Menu

- Next step: a dedicated technical-design pass to produce the Planning Contract (Key Technical Decisions, High-Level Technical Design, Implementation Units), mirroring `docs/plans/2026-09-02-1101-feat-scenario-planner-plan.md`'s depth — this requires reading `src/lib/flow/calc.js`, `src/components/flow/Results.js`, `src/lib/ledger/calc.js`, and `src/app/ledger/page.js` in full, not just the call sites already sampled for this requirements pass.
- Do not begin implementation against this document as-is — it captures WHAT and the settled product-level HOW-shaped decisions (KD1–KD7), but not the file-by-file HOW a Planning Contract provides.
